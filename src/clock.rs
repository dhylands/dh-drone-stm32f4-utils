...
pub struct SystemClock<THR: ThrNvic + ThrFiberFuture> {
...
impl<THR: ThrNvic + ThrFiberFuture> SystemClock<THR> {
    pub fn init(
        flash_acr: reg::flash::Acr<Srt>,
        pwr_cr: reg::pwr::Cr<Srt>,
        rcc_cr: reg::rcc::Cr<Srt>,
    ...
    where
        Self: SystemClockConsts,
    {
        ((Self::HSE_FREQ / self.rcc_pllcfgr.pllm.read_bits()) * self.rcc_pllcfgr.plln.read_bits())
        ...
        Self: SystemClockConsts,
    {
        ...
        self.rcc_pllcfgr.modify(|r| {
            r.set_pllsrc() // HSE oscillator clock selected as PLL input clock
                .write_pllm(Self::PLLM)
                .write_plln(Self::PLLN)
                .write_pllp(Self::PLLP)
                .write_pllq(Self::PLLQ)
        });
        // Enable the PLL.
        ...
        self.flash_acr
            .modify(|r| r.write_latency(Self::FLASH_LATENCY));
        if self.flash_acr.load().latency() != Self::FLASH_LATENCY {
            panic!("LATENCY");
        }
